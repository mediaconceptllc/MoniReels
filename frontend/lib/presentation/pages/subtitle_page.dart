...
class SubtitlePage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final style = project.subtitleStyle;
    final fontsAsync = ref.watch(capabilitiesProvider);
          ...
          Expanded(
            child: Row(
              ...
              children: [
                Expanded(
                  ...
                  child: SingleChildScrollView(
                    child: Column(
                      ...
                      children: [
                        SwitchListTile(
                          title: const Text('Burn subtitles into video'),
                          value: project.export.burnSubtitles,
                          onChanged: (v) => ref
                              .read(currentProjectProvider.notifier)
                              .update((p) => p.copyWith(export: p.export.copyWith(burnSubtitles: v))),
                        ),
                        SwitchListTile(
                          title: const Text('Write .srt sidecar file'),
                          value: project.export.writeSrt,
                          onChanged: (v) => ref
                              .read(currentProjectProvider.notifier)
                              .update((p) => p.copyWith(export: p.export.copyWith(writeSrt: v))),
                        ),
                        const Divider(),
                        fontsAsync.when(
                          data: (caps) => DropdownButtonFormField<String>(
                            ...
                            items: [
                              for (final f in caps.fonts) DropdownMenuItem(value: f, child: Text(f)),
                            ],
                        ...
                        _ColorSwatchRow(
                          ...
                          onSelect: (hex) => updateStyle((s) => s.copyWith(primaryColor: hex)),
                        ),
                        const SizedBox(height: 8),
                        const Text('Outline color'),
                        _ColorSwatchRow(
                          selected: style.outlineColor,
                          onSelect: (hex) => updateStyle((s) => s.copyWith(outlineColor: hex)),
                        ),
                        const SizedBox(height: 12),
                ...
                Expanded(flex: 3, child: _LivePreview(style: style)),
              ],
            ),
          ),
        ],
            ...
            child: Text(
              ...
              style: TextStyle(
                ...
                shadows: [
                  ...
                  if (style.shadow > 0)
                    Shadow(color: Colors.black.withValues(alpha: 0.8), blurRadius: style.shadow, offset: Offset(style.shadow / 2, style.shadow / 2)),
                ],
              ),
            ),
          ),
